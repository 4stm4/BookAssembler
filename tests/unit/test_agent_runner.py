...
class TestInferBySourceReference:
    """The Runner fetches and renders the page itself.

    A caller whose uplink is far slower than the Runner's sends a reference
    instead of a rendered image: a few hundred bytes instead of tens of KB.
    """

    def test_reference_is_rendered_and_inferred(self, client, monkeypatch):
        seen = {}

        def fake_render(url, page, dpi=None):
            seen["url"], seen["page"] = url, page
            return b"\x89PNG rendered"

        monkeypatch.setattr("src.agents.runner.app.render_page", fake_render)
        r = client.post("/infer", json={
            "task": "vision", "source_url": "https://example.org/b.pdf", "page": 4,
        })
        assert r.status_code == 200
        assert seen == {"url": "https://example.org/b.pdf", "page": 4}

    def test_image_upload_still_works(self, client):
        """The old contract must keep working for callers that have not moved."""
        import base64
        r = client.post("/infer", json={
            "task": "vision",
    ...
    def test_neither_form_is_rejected(self, client):
        r = client.post("/infer", json={"task": "vision"})
        assert r.status_code == 422

    def test_unfetchable_source_returns_422_not_500(self, client, monkeypatch):
        """422 tells the caller to fall back to uploading the image."""
        from src.agents.runner.source_fetch import SourceFetchError

        def boom(url, page, dpi=None):
            raise SourceFetchError("refusing to fetch from non-public address")

        monkeypatch.setattr("src.agents.runner.app.render_page", boom)
        r = client.post("/infer", json={
            "task": "vision", "source_url": "http://127.0.0.1/x.pdf", "page": 0,
        })
        assert r.status_code == 422
        assert "non-public" in r.json()["detail"]

    def test_reference_takes_priority_over_image(self, client, monkeypatch):
        import base64
        monkeypatch.setattr("src.agents.runner.app.render_page",
                            lambda url, page, dpi=None: b"\x89PNG from-source")
        r = client.post("/infer", json={
            "task": "vision",
            "source_url": "https://example.org/b.pdf", "page": 0,
            "image_b64": base64.b64encode(b"UPLOADED").decode(),
        })
        assert r.status_code == 200
