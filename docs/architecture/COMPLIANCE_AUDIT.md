...
| 0009 §5.2 | Детерминизм байт-в-байт при повторном прогоне | ✅ | `id` узлов выводятся через `uuid5` от контента (`krm/identity.py`), LLM temp=0/seed=42, из KRM убраны стенные часы, `SOURCE_DATE_EPOCH` пинит PDF. Контракт `test_pipeline_determinism.py`: два полных прогона адаптера+пайплайна по одному файлу дают идентичный KRM-хеш, id включительно. Оговорка: анализаторы с внешней моделью исключены из теста — их детерминизм опирается на temp=0/seed=42 на стороне ollama | — |
...
| 0014 §Idempotency | Двойной прогон анализатора → идентичный AST hash | ✅ | `test_analyzer_idempotency.py` покрывает все 25 анализаторов без исключений (вызов модели застаблен); фикстура содержит заголовок, список, числовую сетку, формулу, callout, библиографию и сноску. Найденный при этом дефект `TableDetector` (повторное слияние tombstoned-блоков) исправлен | — |
...
- ✅ Детерминизм LLM `temperature=0, seed=42` (0012). Идемпотентность (0014) — все 25 анализаторов под контрактом; повторный прогон пайплайна (0009) — идентичный KRM-хеш при выводимых `id`.
