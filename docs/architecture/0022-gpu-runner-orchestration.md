...
`bulk` занимает не более `MANAGER_BULK_SHARE` (default 0.75) одновременных
слотов. Хотя бы один слот всегда свободен для более срочного класса.
...
2. Запрос класса `bulk` не берётся, если это нарушит `MANAGER_BULK_SHARE`.
