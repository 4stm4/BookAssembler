...
|---|---|---|---|
| Accepted | 1.2.0 | 2026-09-03 | Core Architecture Team |
...
>
> **Kaggle backend — основной путь для всех задач (v1.2.0, 2026-09-03).**
> До v1.1.0 на Runner уходили только задачи с картинкой, а `llm_refinement` и
> перевод шли на CPU rpi5 — то есть самая тяжёлая часть конвейера не касалась
> GPU вовсе. Замер: этап уточнения выбирал бюджет в 357 с и пропускал 83 блока,
> пока GPU простаивал между страницами.
>
> v1.2.0 разрешает текстовые задачи (§4.4) и вводит **классы приоритета** (§4.5),
> потому что массовый перевод иначе вытеснит из очереди интерактивный запрос
> пользователя. Модель для этого не добавляется: Qwen2.5-VL-7B уже занимает
> 15000 МБ из 16384 на T4 и умеет текст — второй процесс `ollama serve` рядом
> физически не поднимется.
>
> Edge-кластер (rpi5 + orangepi) остаётся **деградационным путём** для классов
> `bulk`, когда Manager отвечает 429/503 или квота исчерпана (§7.2). См.
> [`docs/deploy/edge-cluster.md`](../deploy/edge-cluster.md).
...
- Держит **очередь запросов с приоритетами** (§4.5, §5.6) — пока Runner
  cold/warming и пока слоты in-flight заняты.
- Реализует **retry, rate-limit, back-pressure**.
- Регистрируется в KAE-agent-manager как обычный агент; агент-роль сохраняется
  (`table`/`formula`/`vision`/`refine`/`translate`) — вся остальная KAE не
  отличает его от single-node агента.
- Ведёт **бюджет GPU-минут** для массовых задач (§7.2).
...
- **Не хранит состояние** между жизнями. Всё, что нужно на повторный запрос —
  приходит с ним (`task` и, по §4.4, `image_b64` либо `prompt`).
...
- `POST /infer` — идентичен Runner'у по контракту:
  `{task, image_b64?, prompt?, priority?}` → `{text}`. Поле `image_b64` стало
  необязательным в v1.2.0: его требуют только задачи с картинкой (§4.4).
  **Дополнительно** Manager соблюдает:
  ...
  - планирует очередь по классам приоритета (§4.5), а не FIFO;
  - отклоняет класс `bulk` с 429, когда исчерпан бюджет GPU-минут (§7.2).
...
- `POST /infer` — `{task, image_b64?, prompt?}` → `{text}`. Runner проверяет
  соответствие задачи и полезной нагрузки (§4.4) и отвечает `400`, если задача
  с картинкой пришла без `image_b64` или текстовая — без `prompt`. Приоритет
  Runner не видит: планирование целиком на Manager'е, Runner исполняет по
  одному запросу.
...
### 4.4 Классы задач и полезная нагрузка

Задача объявляет, что ей нужно. Проверка обязательна на обеих сторонах —
несоответствие это `400`, а не «модель как-нибудь разберётся».

| task | нужна картинка | нужен prompt | что делает |
|---|---|---|---|
| `ocr` | да | нет | распознавание страницы без текстового слоя |
| `vision` | да | нет | разбор и классификация страницы |
| `table` | да | нет | реконструкция таблицы из области |
| `formula` | да | нет | формула → LaTeX |
| `refine` | **нет** | да | уточнение типа блока (`LLMRefinementAnalyzer`) |
| `translate` | **нет** | да | перевод фрагмента (`src/assembler/translator`) |

`ocr` отделён от `vision` намеренно: обе задачи исполняет одна модель, но у них
разная срочность (§4.5) — без OCR страница пуста, а без `vision` она лишь хуже
разобрана. Приоритет назначается по задаче, поэтому различать их нужно в
контракте, а не в вызывающем коде.

`refine` и `translate` обслуживает **та же** Qwen2.5-VL, что уже загружена под
`vision`: она мультимодальна и без картинки работает как обычная текстовая
модель. Отдельная модель не добавляется и в реестр §6 не вносится — иначе LRU
начнёт выгружать зрение ради текста на карте, где свободно 1384 МБ.

### 4.5 Классы приоритета

Без приоритетов правка бессмысленна: перевод книги это тысячи запросов, и
поставленный за ними интерактивный запрос пользователя будет ждать часами.
Класс задаёт **клиент**, но Manager его **ограничивает сверху** по таблице —
иначе любой вызывающий объявит свою пачку интерактивной.

| класс | значение | кто | почему именно так |
|---|---|---|---|
| `interactive` | 0 | «Агент стр.» в редакторе | человек ждёт ответа прямо сейчас |
| `blocking` | 1 | `ocr` | без него у страницы нет текста вообще, весь конвейер ниже пуст |
| `structural` | 2 | `table`, `formula`, `vision` | страница уже существует, задача её улучшает |
| `bulk` | 3 | `refine`, `translate` | тысячи элементов, ни одного никто не ждёт поштучно |

Максимально допустимый класс для задачи: `ocr` → `blocking`, `table`/`formula`/
`vision` → `structural`, `refine`/`translate` → `bulk`. `interactive` выдаётся
только запросу, пришедшему с пользовательского действия, и подтверждается
Manager'ом по признаку вызова, а не по слову клиента.
...
### 5.6 Планирование очереди

Строгий приоритет с двумя ограничителями. Без них строгий приоритет вырождается
в две известные патологии, и обе для нас реальны.

**Голодание.** Пока идёт перевод книги, класс `bulk` при строгом приоритете
может не получить слот ни разу за час. Поэтому **старение**: запрос, ждущий
дольше `MANAGER_AGING_SECONDS` (default 300), поднимается на один класс. Подъём
ограничен: `bulk` дорастает максимум до `structural` и никогда не становится
`interactive` — иначе перевод начнёт вытеснять то, ради чего приоритеты и
вводились.

**Инверсия через занятость.** Если все слоты in-flight заняты `bulk`, пришедший
`interactive` ждёт завершения чужой работы, а не очереди. Поэтому **резерв**:
`bulk` занимает не более `MANAGER_CONCURRENCY - 1` слотов, то есть хотя бы один
всегда остаётся более срочному классу.

Резерв имеет смысл только при `MANAGER_CONCURRENCY ≥ 2`. При одном слоте
зарезервировать нечего: длинный `bulk`-запрос всё равно додержит единственный
слот до конца, вытеснения нет по построению. В этой конфигурации единственный
предохранитель — старение, и это ограничение, а не недосмотр: вытеснение
запущенного inference потребовало бы прерывать генерацию на Runner'е, а он по
§2.2 не хранит состояние и переиграть запрос дешевле, чем откатить.

Правила планировщика:

1. Из очереди берётся запрос с наименьшим номером класса; при равенстве — FIFO
   по времени постановки. Это делает порядок воспроизводимым.
2. Запрос класса `bulk` не берётся, если это заняло бы последний свободный
   слот при `MANAGER_CONCURRENCY ≥ 2`.
3. Старение пересчитывается при каждом выборе, а не по таймеру: так решение
   зависит только от состояния очереди и часов, и его можно воспроизвести в
   тесте.
4. Отмена: клиент, разорвавший соединение, снимается с очереди — перевод
   страницы, которую пользователь уже закрыл, не должен занимать GPU.

Планирование живёт **только в Manager'е**. Runner исполняет по одному запросу и
о приоритетах не знает — иначе состояние размажется по двум процессам, а Runner
по §2.2 не хранит состояние между жизнями.
...
### 7.1 Что отдаёт каждый уровень

Runner в `/metrics` — без метки `priority`: по §5.6 он о классах не знает, и
метрика с меткой, которой у него нет, была бы выдумкой.
...
Manager в `/metrics` — всё, что связано с очередью, живёт здесь, потому что
здесь же живёт планировщик:
- `queue_wait_seconds{priority}` (histogram) — по нему видно голодание класса
- `queue_depth{priority}`
- `infer_requests_total{task,priority}`
- `bulk_minutes_used`, `gpu_quota_exhausted`
...
### 7.2 Бюджет GPU-минут

Free-tier это ≈30 GPU-часов в неделю (§1). Перевод книги способен выбрать их
целиком, и тогда OCR следующего документа встанет — а OCR это класс `blocking`,
без которого страница пуста.

Поэтому у класса `bulk` **отдельный бюджет**: `MANAGER_BULK_BUDGET_MINUTES`
(default 60% недельной квоты). При исчерпании Manager отвечает на `bulk` кодом
`429` с `Retry-After`, продолжая обслуживать `interactive`, `blocking` и
`structural`. KAE в этом случае уводит `refine` и `translate` на edge-кластер —
медленно, но результат есть.

Инвариант: **исчерпание бюджета `bulk` никогда не отказывает `blocking`.** Если
для этого не хватает квоты вовсе, Manager отдаёт `503` и в `/metrics`
поднимает `gpu_quota_exhausted` — состояние, которое видно оператору, а не
проявляется молча пустыми страницами.
...
6. **Детерминизм LLM (RFC 0012 §3.1):** Runner соблюдает `temperature=0, seed=42`
   как раньше; Manager не переопределяет. Приоритет влияет на **порядок**
   обслуживания и никогда — на результат: два одинаковых запроса, поданных с
   разными классами, дают идентичный ответ (RFC 0009 §5.2).
7. **Полезная нагрузка проверяется (v1.2.0):** задача с картинкой без
   `image_b64` и текстовая без `prompt` — это `400` на обеих сторонах. Молча
   передать модели половину запроса нельзя.
8. **Приоритет ограничивается сервером:** клиент предлагает класс, Manager
   опускает его до максимально допустимого для задачи (§4.5). Клиент не может
   объявить массовый перевод интерактивным.
9. **Голодание ограничено по времени:** любой запрос повышается в классе после
   `MANAGER_AGING_SECONDS` ожидания, а `bulk` никогда не занимает все слоты
   in-flight (§5.6). Строгий приоритет без этих двух ограничителей не
   деплоится.
10. **Бюджет `bulk` не отказывает `blocking`:** исчерпание бюджета массовых
    задач останавливает только их (§7.2). Если квоты не хватает и на
    `blocking` — это `503` и `gpu_quota_exhausted` в метриках, а не тихо
    пустые страницы.
11. **Текстовые задачи не добавляют модель:** `refine` и `translate` исполняет
    модель, уже загруженная под `vision`. Отдельная текстовая модель на карте,
    где свободно 1384 МБ из 16384, вытеснит зрение через LRU (§6).
